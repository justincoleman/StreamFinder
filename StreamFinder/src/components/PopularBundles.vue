<template>
  <div class="bg-gradient-to-br from-purple-50 to-blue-50 dark:from-slate-800 dark:to-slate-700 border-2 border-purple-200 dark:border-purple-400 p-6 rounded-lg">
    <!-- Header -->
    <div class="text-center mb-8">
      <div class="flex items-center justify-center gap-3 mb-2">
        ...
        <h2 class="text-3xl font-bold font-display text-purple-800 dark:text-purple-200 uppercase tracking-wide">
          Popular Bundles
        </h2>
      </div>
      <p class="text-lg text-purple-600 dark:text-purple-300 font-sans">
    ...
    <div v-if="isLoading" class="flex items-center justify-center py-12">
      <div class="flex items-center gap-3">
        <svg class="animate-spin h-6 w-6 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
        </svg>
        <span class="text-purple-600 dark:text-purple-400 font-medium">Loading popular bundles...</span>
      </div>
    </div>
    ...
    <div v-else-if="topBundles.length > 0" class="relative">
      ...
      <div v-if="topBundles.length < 3" class="text-center mb-6">
      ...
      <div class="flex items-end justify-center gap-4 mb-8">
        ...
        <div v-if="topBundles[1]" class="flex flex-col items-center">
          <div class="bg-white dark:bg-slate-800 border-2 border-gray-400 rounded-lg p-4 w-48 mb-4 shadow-lg transform hover:scale-105 transition">
            <div class="text-center mb-3">
              <div class="text-lg font-bold text-gray-700 dark:text-gray-300 mb-2">
          ...
          <div class="w-32 h-16 bg-gradient-to-t from-gray-400 to-gray-300 border-2 border-gray-500 rounded-t-lg flex items-center justify-center">
            <span class="text-white font-bold text-sm">2nd</span>
          </div>
        </div>

        <!-- 1st Place (Center, Taller) - Always show if any data exists -->
        <div v-if="topBundles[0]" class="flex flex-col items-center">
          <div class="bg-white dark:bg-slate-800 border-2 border-yellow-400 rounded-lg p-6 w-56 mb-4 shadow-xl transform hover:scale-105 transition">
            <div class="text-center mb-4">
              <div class="text-xl font-bold text-gray-700 dark:text-gray-300 mb-3">
            ...
            <div class="space-y-3">
              ...
              <div class="text-sm text-center text-gray-600 dark:text-gray-400 font-medium">
                {{ topBundles[0].popularity }} users chose this
              </div>
            </div>
          </div>
          <div class="w-32 h-24 bg-gradient-to-t from-yellow-500 to-yellow-400 border-2 border-yellow-600 rounded-t-lg flex items-center justify-center">
            <span class="text-white font-bold">{{ topBundles.length === 1 ? '🥇' : '1st' }}</span>
          </div>
        </div>

        <!-- 3rd Place (Right) - Only show if exists -->
        <div v-if="topBundles[2]" class="flex flex-col items-center">
          <div class="bg-white dark:bg-slate-800 border-2 border-amber-600 rounded-lg p-4 w-48 mb-4 shadow-lg transform hover:scale-105 transition">
            <div class="text-center mb-3">
              <div class="text-lg font-bold text-gray-700 dark:text-gray-300 mb-2">
          ...
          <div class="w-32 h-12 bg-gradient-to-t from-amber-700 to-amber-600 border-2 border-amber-700 rounded-t-lg flex items-center justify-center">
            <span class="text-white font-bold text-sm">3rd</span>
          </div>
        </div>
      </div>
      ...
      <div class="text-center">
        <button @click="refreshBundles" :disabled="isRefreshing"
          class="px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white font-bold font-display uppercase tracking-wider rounded-lg transition disabled:opacity-50">
          <svg v-if="!isRefreshing" class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
          <svg v-else class="w-5 h-5 inline mr-2 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
    ...
    <div v-else class="text-center py-12">
      ...
      <h3 class="text-lg font-bold font-display text-gray-600 dark:text-gray-400 mb-2">No Data Yet</h3>
