<template>
  <div
    class="rounded-2xl shadow-xl p-6 md:p-8 mb-8 border border-slate-700/50 bg-slate-700/80 backdrop-blur-md transition-all duration-300 relative h-full flex flex-col animated-glow ring-1 ring-emerald-400/30 [box-shadow:0_0_16px_2px_rgba(16,185,129,0.25)]"
    :class="[
      ...
      isSecondaryOption && item.isSubscribed ? 'bg-slate-900/60 border-emerald-400/20' :
  ...
  >
    ...
    <div class="pt-5 flex-grow flex flex-col">
      <div v-if="item.type === 'bundle'" class="mb-3 text-center">
        <h3 class="text-xl sm:text-2xl font-bold mb-3 bg-gradient-to-r from-blue-400 to-indigo-500 bg-clip-text text-transparent"
            :class="{
        ...
        <div class="flex flex-wrap justify-center items-center gap-x-1.5 gap-y-2 p-2 rounded-md shadow-inner bg-slate-900/40">
      ...
      <h3 v-else class="text-xl sm:text-2xl md:text-3xl font-bold text-center mb-3 bg-gradient-to-r from-blue-400 to-indigo-500 bg-clip-text text-transparent"
          :class="{
      ...
      <div class="text-center my-3 py-3 border-y border-slate-700/40">
        <p class="text-xs sm:text-sm text-slate-400 mb-0.5">Total Monthly Cost {{ item.type === 'bundle' ? 'for this Bundle' : 'for this Service' }}:</p>
        <div class="inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-br from-blue-500/20 to-indigo-600/20">
          <span class="px-3 py-1.5 bg-slate-900/60 text-lg sm:text-xl md:text-2xl font-bold rounded-full shadow-inner text-slate-100">
            ${{ item.totalNumericPrice != null && isFinite(item.totalNumericPrice) ? item.totalNumericPrice.toFixed(2) : '0.00' }}
          </span>
        </div>
        <p v-if="(item.additionalNumericCost > 0 || (item.type === 'bundle' && userHasSubscriptions)) && isFinite(item.additionalNumericCost)" class="text-xs sm:text-sm text-emerald-400 mt-1.5">
          Additional cost: <span class="font-semibold">${{ item.additionalNumericCost.toFixed(2) }}</span>
        </p>
         <p v-else-if="item.totalCoveredLeaguesCount > 0 && item.additionalNumericCost === 0 && !userHasSubscriptions && item.type === 'bundle'" class="text-xs sm:text-sm text-emerald-400 mt-1.5">
      ...
      <div v-if="item.badge === 'Top Coverage' && isPrimaryRecommendation && item.redundantSubscriptions && item.redundantSubscriptions.length > 0"
           class="my-3 p-3 bg-orange-100/20 border border-orange-300/40 rounded-lg shadow-sm">
        <button @click="savingsVisible = !savingsVisible" class="w-full text-left focus:outline-none group block min-h-[44px]">
          <div class="flex justify-between items-center">
            <h4 class="text-sm font-semibold text-orange-400 group-hover:text-orange-500">Potential Savings!</h4>
          ...
          <p class="text-xs text-orange-300 group-hover:text-orange-400">
        ...
        <transition name="fade-slide">
          <div v-show="savingsVisible" class="mt-2 pt-2 border-t border-orange-200/40">
            <p class="text-xs text-orange-200 mb-1">By potentially unsubscribing from:</p>
            <ul class="list-disc pl-5 space-y-0.5 text-xs">
              <li v-for="sub in item.redundantSubscriptions" :key="sub.id" class="text-orange-200">
      ...
      <div class="grid md:grid-cols-2 gap-x-4 mb-3 items-center">
        <div>
            <h4 class="text-sm font-semibold text-slate-200 mb-0.5">Leagues Covered:</h4>
          <p class="text-lg sm:text-xl font-bold text-slate-100 break-words">
            {{ item.totalCoveredLeaguesCount }}
            <span class="text-xs sm:text-sm font-normal text-slate-400">of {{ store.selectedLeagueIds.length }}</span>
            </p>
        </div>
        <div v-if="isPrimaryRecommendation && item.badge === 'Top Coverage' && item.totalCoveredLeaguesCount < store.selectedLeagueIds.length" class="flex flex-col items-end justify-center w-full">
          <span class="text-sm font-semibold text-orange-300">Missing:</span>
          <p class="text-lg sm:text-sm font-bold text-orange-400 mt-0.5 text-right break-words">
        ...
        <div v-if="item.type === 'bundle' && Object.keys(item.newlyCoveredLeaguesDetails || {}).length > 0">
            <h4 class="text-sm font-semibold text-blue-300 mb-0.5">Newly Added:</h4>
            <p class="text-lg sm:text-xl font-bold text-blue-300">
                {{ Object.keys(item.newlyCoveredLeaguesDetails).length }} <span class="text-xs sm:text-sm font-normal text-blue-200">Leagues</span>
            </p>
        </div>
      </div>
      <div v-if="item.type === 'bundle' && item.servicesInvolved.length > 0" class="mt-2 mb-auto">
        <h4 class="text-md font-semibold text-slate-200 mb-1.5">Services in this Bundle:</h4>
        <ul class="list-none pl-0 space-y-2">
          <li v-for="service in item.servicesInvolved" :key="service.id"
              class="p-2 rounded-md flex flex-col sm:flex-row justify-between items-start sm:items-center shadow-sm border border-slate-700/40 bg-slate-900/40 gap-y-1"
              :class="service.isSubscribed
                        ? 'border-emerald-400/40 bg-emerald-900/10'
                        : (isPrimaryRecommendation && item.badge === 'Top Coverage' ? 'border-yellow-400/40 bg-yellow-900/10' : 'border-slate-700/40 bg-slate-900/40')
                     "
          >
            <div>
              <strong class="text-sm sm:text-base font-medium text-slate-100 break-words">{{ service.name }}</strong>
              <span class="text-xs sm:text-sm block sm:inline sm:ml-1 text-slate-400">({{ service.price }})</span>
              <span v-if="service.isSubscribed" class="text-xs font-bold ml-1 text-emerald-400">(Subscribed)</span>
              <span v-if="!service.isSubscribed && isPrimaryRecommendation && item.badge === 'Top Coverage'" class="text-xs font-bold ml-1 text-yellow-300">(New in this bundle)</span>
            </div>
            <a
