      ...
      <ul>
        <li v-for="category in store.allLeaguesByCategory" :key="category.categoryName" class="border border-slate-200 dark:border-slate-700/50 rounded-xl shadow-lg overflow-hidden bg-slate-100/80 dark:bg-slate-800/80 backdrop-blur-sm mb-9">
          <button
            ...
            :aria-expanded="store.expandedCategories.includes(category.categoryName)"
            :aria-controls="`category-panel-${category.categoryName}`"
          >
            <div class="flex items-center">
              <span class="mr-2 text-xl" aria-hidden="true">{{ category.icon || '🏆' }}</span>
          ...
          >
            <div v-show="store.expandedCategories.includes(category.categoryName)" :id="`category-panel-${category.categoryName}`" class="p-4 bg-slate-50 dark:bg-slate-800/20">
              <ul class="space-y-2">
                <li v-for="league in category.leagues" :key="league.id">
                  <button
                    ...
                    :aria-pressed="store.selectedLeagueIds.includes(league.id)"
                  >
