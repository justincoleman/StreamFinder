  ...
  <div>
    ...
    <div class="mb-6 flex space-x-3">
      <button
        ...
        class="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-150"
      >
        Select All Leagues
      </button>
      <button
    ...
    <div class="space-y-4">
      <div v-for="category in store.allLeaguesByCategory" :key="category.categoryName" class="border border-gray-200 rounded-lg shadow-sm overflow-hidden">
        ...
        >
          <div v-show="store.expandedCategories.includes(category.categoryName)" class="p-4 bg-white">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              <button
                ...
                :class="[
                  'px-3 py-2 rounded-md text-sm font-medium transition-all duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 flex items-center justify-start text-left w-full shadow-sm hover:shadow-md',
                  store.selectedLeagueIds.includes(league.id)
                    ? 'bg-blue-600 text-white hover:bg-blue-700 focus:ring-blue-500'
                    : 'bg-gray-100 text-gray-700 hover:bg-gray-200 focus:ring-blue-400'
                ]"
              >
                <span class="text-lg mr-2 w-5 text-center">{{ league.icon }}</span>
                {{ league.name }}
              </button>
            </div>
          </div>
