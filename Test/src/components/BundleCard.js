<template>
  <div
    ...
    :class="[
      isPrimaryRecommendation && item.badge === 'Top Coverage' ? 'border-2 border-green-500 bg-green-50 ring-green-400' + (isMainPick ? ' md:scale-105 z-10' : '') :
      isPrimaryRecommendation && item.badge === 'Best Value' ? 'border-2 border-teal-500 bg-teal-50 ring-teal-400' :
      isSecondaryOption && item.isSubscribed ? 'bg-gray-50 border-gray-200 ring-gray-200' : // Muted for subscribed secondary options
      'bg-white border-gray-200 ring-gray-200' // Default
    ]"
  >
    <div
    ...
    <div class="pt-5 flex-grow flex flex-col">
      ...
      <h3 v-else class="text-lg sm:text-xl md:text-2xl font-bold text-center mb-3"
          :class="{ /* Styling for individual service titles if they get a primary badge */
      ...
      <div v-if="item.badge === 'Top Coverage' && isPrimaryRecommendation && item.redundantSubscriptions && item.redundantSubscriptions.length > 0"
        ...
        <transition name="fade-slide">
          <div v-show="savingsVisible" class="mt-2 pt-2 border-t border-orange-100">
            <p class="text-xs text-gray-500 mb-1">You could potentially unsubscribe from:</p>
      ...
      <div v-if="item.type === 'bundle' && item.servicesInvolved.length > 0" class="mt-2 mb-auto">
        ...
        <ul class="list-none pl-0 space-y-1.5">
          <li v-for="service in item.servicesInvolved" :key="service.id"
              ...
              :class="service.isSubscribed
                        ? 'bg-white border-green-300' /* White background for subscribed services in any bundle */
                        : (isPrimaryRecommendation && item.badge === 'Top Coverage' ? 'bg-yellow-100 border-yellow-400' : 'bg-gray-100 border-gray-200') /* Yellow for new in Top Coverage, gray otherwise */
                     "
          >
            <div>
